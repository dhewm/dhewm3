...
void idAFEntity_VehicleSimple_4wd::Spawn( void ) {
	...
	trm.Translate( idVec3( 0, 0, -wheelRadius ) );
	wheelModel = new idClipModel( trm );
	//Added for getting suspension var's from .def file
	spawnArgs.GetFloat( "suspensionDamp", "400", suspensionDamp );
	spawnArgs.GetFloat( "suspensionKComp", "200", suspensionKComp );
	spawnArgs.GetFloat( "suspensionUpMotion", "32", suspensionUpMotion );
	spawnArgs.GetFloat( "suspensionDownMotion", "20", suspensionDownMotion );
	spawnArgs.GetFloat( "suspensionTireFriction", "0.8", suspensionTireFriction );
	...
	for ( i = 0; i < 4; i++ ) {
		...
		suspension[i] = new idAFConstraint_Suspension();
		suspension[i]->Setup( va( "suspension%d", i ), af.GetPhysics()->GetBody( 0 ), origin, af.GetPhysics()->GetAxis( 0 ), wheelModel );
		suspension[i]->SetSuspension(	suspensionUpMotion,
										suspensionDownMotion,
										suspensionKComp,
										suspensionDamp,
										suspensionTireFriction );
...
void idAFEntity_VehicleSimple_4wd::Think( void ) {
	...
	if ( thinkFlags & TH_THINK ) {
		...
		if ( player ) {
			// capture the input from a player
			velocity = vehicleVelocity;			//g_vehicleVelocity.GetFloat();
			if ( player->usercmd.forwardmove < 0 ) {
				velocity = -velocity;
			}
			force = idMath::Fabs( player->usercmd.forwardmove *  vehicleForce) * (1.0f / 128.0f);		//g_vehicleForce.GetFloat()
			steerAngle = GetSteerAngle();
		}
