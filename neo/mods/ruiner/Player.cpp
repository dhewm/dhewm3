...
void idPlayer::UpdateHudStats( idUserInterface *_hud ) {
	...
	if(weapon.GetEntity()->GetIsFiring() || weapon.GetEntity()->GetIsSecFiring() || force_torso_override ){ //firing or combo
...
void idPlayer::UpdateConditions( void ) {
	...
	*/
	...
	AI_RUN = ( weapon.GetEntity() ) ? (!weapon.GetEntity()->HasToWalk()) : true;
	//ivan end
...
void idPlayer::AdjustSpeed( void ) {
	...
	//ivan start
	/* 
	//ivan: commented out the run button stuff

	//was: } else if ( !physicsObj.OnLadder() && ( usercmd.buttons & BUTTON_RUN ) && ( usercmd.forwardmove || usercmd.rightmove ) && ( usercmd.upmove >= 0 ) ) {
