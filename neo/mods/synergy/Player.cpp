...
// <---sikk
	...
	v3CrosshairPos			= idVec3( 0.0f, 0.0f, 0.0f );	// sikk - Thirdperson Crosshair
...
// sikk---> Regenerative Health System
			if ( g_useHealthRegeneration.GetBool() )
				g_armorProtection.SetFloat( ( g_skill.GetInteger() < 2 ) ? 0.75f : 0.5f );
			else
...
// <---sikk
	...
	savefile->WriteBool( bAmbientLightOn );	// sikk - Global Ambient Light
...
// <---sikk
	...
	savefile->ReadBool( bAmbientLightOn );	// sikk - Global Ambient Light
...
// sikk---> Weapon Awareness
	} else if ( g_useWeaponAwareness.GetBool() ) {
		idEntity *ent;
		trace_t trace;
		idVec3 start = GetEyePosition();
		idVec3 end = start + viewAngles.ToForward() * g_weaponAwarenessDistance.GetInteger();
		gameLocal.clip.TracePoint( trace, start, end, ( MASK_SHOT_RENDERMODEL | CONTENTS_OPAQUE ), this );
		bWATrace = false;
		if ( ( trace.fraction < 1.0f ) && ( GetCurrentWeapon() > 0 && GetCurrentWeapon() < 9 ) ) {
			...
			if ( ent && !ent->IsType( idAI::Type ) ) 
				...
				if ( ent->GetBindMaster() != NULL )
					if ( ent->GetBindMaster()->IsType( idAI::Type ) )
						bWATrace = false;
		}

		bWAIsSprinting = ( AI_RUN && ( AI_FORWARD || AI_BACKWARD || AI_STRAFE_LEFT || AI_STRAFE_RIGHT ) );
...
bool idPlayer::HandleSingleGuiCommand( idEntity *entityGui, idLexer *src ) {
	...
	if ( token.Icmp( "addhealth" ) == 0 ) {
		if ( entityGui && health < 100 ) {
			...
			prevHeatlh = health;	// sikk - Regenerative Health System
...
// sikk---> Depth of Field PostProcess
	if ( renderView ) {
		if ( gameLocal.inCinematic ) {
			start = GetRenderView()->vieworg;
			end = start + GetRenderView()->viewaxis.ToAngles().ToForward() * 8192.0f;
		} else {
			start = GetEyePosition();
	//		idAngles tmpAngles = viewAngles + playerView.AngleOffset();
	//		end = start + tmpAngles.ToForward() * 8192.0f;
			end = start + viewAngles.ToForward() * 8192.0f;
		}
//		gameRenderWorld->DebugArrow( colorYellow, start, end, 10, 0 );
		gameLocal.clip.TracePoint( trace, start, end, ( MASK_SHOT_RENDERMODEL | CONTENTS_OPAQUE ), this );
...
void idPlayer::SetCurrentHeartRate( void ) {
	...
	if ( PowerUpActive( ADRENALINE ) ) {
		heartRate = 135;
// sikk---> Regenerative Health System
	} else if ( g_useHealthRegeneration.GetBool() && health < g_healthRegenLimit.GetInteger() && health > 0 ) {
...
// <---sikk
	...
	if ( gameLocal.time - lastHeartBeat > bps ) {
		...
		if ( pct != zeroVol ) {
// sikk---> Regenerative Health System
			if ( g_useHealthRegeneration.GetBool() && health < g_healthRegenLimit.GetInteger() && health > 0 ) {
...
// sikk---> Crosshair Cvar
	if ( g_crosshair.GetInteger() == 0 ) {
		cursor->SetStateString( "combatcursor", "0" );
	} else if ( g_crosshair.GetInteger() == 1 ) {
		cursor->SetStateString( "combatcursor", "1" );
		cursor->SetStateString( "cursorposition", buf );
	} else if ( g_crosshair.GetInteger() == 2 ) {
		cursor->SetStateString( "cursorposition", buf );
		if ( ( bIsZoomed || GetTalkCursor() ) &&
			!( g_useWeaponAwareness.GetBool() && ( bWATrace || bWAIsSprinting || OnLadder() ) ) &&
...
void idPlayer::Think( void ) {
	...
	if ( ( usercmd.buttons ^ oldCmd.buttons ) & BUTTON_ZOOM ) {
		if ( ( usercmd.buttons & BUTTON_ZOOM ) && weapon.GetEntity() /*&& ( g_useWeaponAwareness.GetBool() && AI_RUN )*/ ) {	// sikk - Weapon Awareness
...
// sikk---> First Person Body
	idStr skin = spawnArgs.GetString( "spawn_skin" );

	if ( g_showFirstPersonBody.GetBool() && !pm_thirdPerson.GetBool() && !gameLocal.inCinematic ) {
		if ( skin == "skins/characters/player/tshirt_mp" ) {
		...
		if ( skin == "skins/characters/player/tshirt_mp" ) {
			SetSkin( declManager->FindSkin( skin ) );
		} else {
...
void idPlayer::ToggleIRGoggles() {
	...
	if ( bIRGogglesOn ) {
		...
		if ( !g_goggleType.GetBool() ) {
			...
			ent->SetAngles( GetPhysics()->GetAxis().ToAngles() );	// light angle
		}
