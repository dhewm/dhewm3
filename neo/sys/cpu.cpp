...
// mingw-w64 changed __cpuid to the same type as msvc located in intrinsics.h so we cannot use this one
// atleast not on windows since SDL includes intrin.h before it so it gets overridden
#if defined(__unix__) || defined(MACOS_X) || defined(__APPLE__) || defined(__AROS__)
#include <cpuid.h>
#endif
...
// guessing here but i reckon most unixes use the same function from cpuid.h
#if defined(__unix__) || defined(MACOS_X) || defined(__APPLE__) || defined(__AROS__)
    __cpuid( func, regs[_REG_EAX], regs[_REG_EBX], regs[_REG_ECX], regs[_REG_EDX] );
#else /* msvc / mingw / clang */
    int values[4] = { 0, 0, 0, 0 };

    __cpuid( values, func );

    regs[_REG_EAX] = static_cast<unsigned>( values[0] );
    regs[_REG_EBX] = static_cast<unsigned>( values[1] );
    regs[_REG_ECX] = static_cast<unsigned>( values[2] );
    regs[_REG_EDX] = static_cast<unsigned>( values[3] );
#endif
}
...
static inline void EnableMXCSRFlag( unsigned int flag, bool enable, const char *name ) {
	...
	// prevent resetting flag if allready enabled
    if ( enable && ( sse_mode & flag ) == flag ) {
        common->Printf( "%s mode is already enabled\n", name );
        return;
    }

    // prevent resetting flag if allready disabled
...
void Sys_FPU_SetDAZ( bool enable ) {
	...
	// extra safeguard simd.cpp does check for sse mode but not daz
...
void Sys_FPU_SetFTZ( bool enable ) {
	...
	// extra safeguard simd.cpp does check for sse mode so probably overkill
