...
void idImage::Generate3DImage( const byte *pic, int width, int height, int picDepth,
    ...
    qglTexImage3D( GL_TEXTURE_3D, 0, internalFormat, scaled_width, scaled_height, scaled_depth, 0, GL_RGBA, GL_UNSIGNED_BYTE, pic );
...
void idImage::GenerateCubeImage( const byte *pic[6], int size,
    ...
    while ( scaled_width > 1 ) {
        for ( i = 0 ; i < 6 ; i++ ) {
            ...
            qglTexImage2D( GL_TEXTURE_CUBE_MAP_POSITIVE_X_EXT + i, miplevel, internalFormat, scaled_width / 2, scaled_height / 2, 0,
...
void idImage::CopyFramebuffer( int x, int y, int imageWidth, int imageHeight, bool useOversizedBuffer ) {
    ...
    if ( ( useOversizedBuffer && ( uploadWidth < potWidth || uploadHeight < potHeight ) ) ||
        ...
        if ( potWidth == imageWidth && potHeight == imageHeight ) {
            ...
            dummyimg = ( byte * )Mem_Alloc( potWidth * potHeight * 4 );
            memset( dummyimg, 0, potWidth * potHeight * 4 );        //!@#
            qglTexImage2D( GL_TEXTURE_2D, 0, GL_RGB, potWidth, potHeight, 0, GL_RGBA, GL_UNSIGNED_BYTE, dummyimg );
            Mem_Free( dummyimg );
