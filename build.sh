...
echo "Checking if there in missing libs ..."

packages_apt=("git" "cmake" "build-essential" "libsdl2-dev" "libopenal-dev" "libcurl4-openssl-dev")
packages_pacman=("git" "cmake" "base-devel" "sdl2" "openal" "curl")
packages_dnf=("git" "cmake" "gcc-c++" "make" "SDL2-devel" "openal-soft-devel" "libcurl-devel")
packages_zypper=("git" "cmake" "gcc-c++" "make" "libSDL2-devel" "openal-soft-devel" "libcurl-devel")
missingLib=false
...
if command -v apt >/dev/null 2>&1; then
    ...
    for pkg in "${packages_zypper[@]}"; do
        if ! zypper search -i --match-exact "$pkg" >/dev/null 2>&1; then
            echo "Missing: $pkg"
            missingLib=true
        fi
    done
